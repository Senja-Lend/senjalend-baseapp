...
const WithdrawTab = ({ pool }: WithdrawTabProps) => {
  ...
  const { supplyApyFormatted, apyLoading, refetchApy } = useReadApy(
    pool?.lendingPool || "0x0000000000000000000000000000000000000000"
  );
  ...
  const handleSetMaxCollateral = useCallback(() => {
    if (
      collateralBalanceParsed > 0 &&
      collateralBalanceFormatted !== undefined
    ) {
      ...
      <div className="space-y-6">
        ...
        >
          ...
          <PoolInfoCard
            ...
            apy={apyLoading ? "Loading..." : supplyApyFormatted}
